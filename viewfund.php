...
if(isset($_POST['Del_Multi'])){
    ...
    $seprate_all_id = implode(",",$all_id);
	
	$sql = "DELETE FROM fund_raiser WHERE fund_raiser_id IN($seprate_all_id)";
	$qsql = mysqli_query($con,$sql);
	if(mysqli_affected_rows($con) == 1)
	{
		echo "<script>alert('fund record deleted successfully..');</script>";
...
<table id="datatable"class="table table-striped table-bordered">
	<thead>
		<tr>
			<th scope="col">
				<button type="submit" class='btn btn-danger' name="Del_Multi" onclick='return confirmdel()'>Delete</button>
			</th>
			<th>Banner</th>			
			<th>Title</th>			
	...
	{
		$id=$rs['fund_raiser_id'];
				
		echo '<tr>
		<td>
		<input type="checkbox" name="del_chk[]" value="'.$id.'">
		</td>';
